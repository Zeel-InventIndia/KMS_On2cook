...
export function UnifiedDashboard({
  ...
  return (
    <div className="min-h-screen bg-background">
                            ...
                            <tr key={timeSlot} className={timeIndex % 2 === 0 ? 'bg-background' : 'bg-muted/20'}>
                              ...
                              {TEAM_GROUPS.map((team) => {
                                ...
                                return (
                                  <td 
                                    key={team.id} 
                                    className={`p-1 border-l text-center relative ${
                                  ...
                                  >
                                    {assignedDemo ? (
                                      <div className="p-2 bg-white border rounded text-xs">
                                        <div className="font-medium truncate">{assignedDemo.clientName}</div>
                                        <div className="text-muted-foreground">{assignedDemo.assignee}</div>
                                        <Badge 
                                          className={`${getStatusColor(assignedDemo.leadStatus)} text-xs mt-1`}
                                          variant="outline"
                                        >
      ...
      {/* 🔥 FIXED: Missing Modals - This was the issue! */}
      {showDemoDetail && selectedDemo && (
        <DemoDetailModal
          demo={selectedDemo}
          user={user}
          onClose={() => {
            console.log('🔒 Closing demo detail modal');
            setShowDemoDetail(false);
            setSelectedDemo(null);
          }}
          onUpdateDemo={onUpdateDemoRequest}
        />
      )}
